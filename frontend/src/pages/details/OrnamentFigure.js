...
const OrnamentFigure = () => {
    ...
    const createHandler = async () => {
        try {
            const { data: res } = await Api.post('/ornament_figure/create', { ...data })
            setDatas([res, ...datas])
            clearData()
        } catch (error) {
            console.log(data);
    ...
    const updateHandler = async () => {
        try {
            const { data: res } = await Api.put(`/ornament_figure/update`, { ...data })
    ...
    const deleteHandler = async () => {
        try {
            const { data: res } = await Api.delete(`/ornament_figure/delete/${data?._id}`)
    ...
    const getDatas = async () => {
        try {
            const { data } = await Api.get('/ornament_figure/get')
            setDatas(data)
        } catch (error) {
            console.log(error);
        }
        ...
        <div className='max-w-[1800px] mx-auto'>
            <div className='max-w-[1400px] mx-auto p-4'>
                <div className='px-4 mb-[50px]'>
                    <div>
                        <h1 className='text-[32px] font-bold text-center'>Naqsh shakli</h1>
                    </div>
