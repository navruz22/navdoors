...
const Depth = () => {
    ...
    const createHandler = async () => {
        try {
            const {data: res} = await Api.post('/depth/create', {...data}) 
            setDatas([res, ...datas])
            clearData()
        } catch (error) {
            console.log(data);
    ...
    const updateHandler = async () => {
        try {
            const {data: res} = await Api.put(`/depth/update`, {...data}) 
    ...
    const deleteHandler = async () => {
        try {
            const {data: res} = await Api.delete(`/depth/delete/${data?._id}`) 
    ...
    const getDatas = async () => {
        try {
            const { data } = await Api.get('/depth/get')
            setDatas(data)
        } catch (error) {
            console.log(error);
        }
        ...
        <div className='max-w-[1800px] mx-auto'>
            <div className='max-w-[1400px] mx-auto p-4'>
                <div className='px-4 mb-[50px]'>
                    <div>
                        <h1 className='text-[32px] font-bold text-center'>Karobka qalinligi</h1>
                    </div>
