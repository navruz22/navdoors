...
const FramogType = () => {
    ...
    const createHandler = async () => {
        try {
            const {data: res} = await Api.post('/framog_type/create', {...data}) 
            setDatas([res, ...datas])
            clearData()
        } catch (error) {
            console.log(data);
    ...
    const updateHandler = async () => {
        try {
            const {data: res} = await Api.put(`/framog_type/update`, {...data}) 
    ...
    const deleteHandler = async () => {
        try {
            const {data: res} = await Api.delete(`/framog_type/delete/${data?._id}`) 
    ...
    const getDatas = async () => {
        try {
            const { data } = await Api.get('/framog_type/get')
            setDatas(data)
        } catch (error) {
            console.log(error);
        }
        ...
        <div className='max-w-[1800px] mx-auto'>
            <div className='max-w-[1400px] mx-auto p-4'>
                <div className='px-4 mb-[50px]'>
                    <div>
                        <h1 className='text-[32px] font-bold text-center'>Framog turi</h1>
                    </div>
