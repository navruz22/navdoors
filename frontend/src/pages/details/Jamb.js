...
const Jambs = () => {
    ...
    const createHandler = async () => {
        try {
            const { data: res } = await Api.post('/jamb/create', { ...data })
            setDatas([res, ...datas])
            clearData()
        } catch (error) {
            console.log(data);
    ...
    const updateHandler = async () => {
        try {
            const { data: res } = await Api.put(`/jamb/update`, { ...data })
    ...
    const deleteHandler = async () => {
        try {
            const { data: res } = await Api.delete(`/jamb/delete/${data?._id}`)
    ...
    const getDatas = async () => {
        try {
            const { data } = await Api.get('/jamb/get')
            setDatas(data)
        } catch (error) {
            console.log(error);
        }
        ...
        <div className='max-w-[1800px] mx-auto'>
            <div className='max-w-[1400px] mx-auto p-4'>
                <div className='px-4 mb-[50px]'>
                    <div>
                        <h1 className='text-[32px] font-bold text-center'>Nalichnik</h1>
                    </div>
